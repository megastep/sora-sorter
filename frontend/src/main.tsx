...
import './layout.css';
const API = '/api';
type ReviewStatus = 'unreviewed' | 'shortlisted' | 'approved' | 'rejected';
type Filters = Record<string, string>;
type LikenessReference = { name: string; confidence: 'possible' | 'likely'; basis: string };
type Video = {
  id: string;
  title: string;
  summary: string;
...
const references = (value: unknown): LikenessReference[] =>
  Array.isArray(value)
    ? value
        .map(referenceObject)
...
function ReferenceEditor({
  ...
  onChange: (value: LikenessReference[]) => void;
}) {
  const update = (index: number, key: keyof LikenessReference, value: string) =>
    onChange(
  ...
  return (
    <div className="references">
      {references.map((reference, index) => (
        <fieldset className="reference" key={index}>
          <input
          ...
          <select
            ...
            onChange={(e) => update(index, 'confidence', e.target.value)}
          >
      ...
      <button
        ...
        onClick={() => onChange([...references, { name: '', confidence: 'possible', basis: '' }])}
      >
        Add reference
      </button>
    </div>
...
function Inspector({ item, onSaved }: { item: Video | null; onSaved: (value: Video) => void }) {
  ...
  if (!draft) return <section className="inspector empty">Select a clip to inspect it.</section>;
  ...
  return (
    <section className="inspector">
      ...
      <label>
        Language
        <input value={draft.language ?? ''} onChange={(e) => set('language', e.target.value)} />
      </label>
      <label>
        Summary
      ...
      <details>
        ...
        <textarea value={draft.transcript} onChange={(e) => set('transcript', e.target.value)} />
      </details>
      <details>
      ...
      <label className="check">
        <input
          ...
          onChange={(e) => set('publishable', e.target.checked)}
        />{' '}
        Publishable
      </label>
      <label>
...
function App() {
  ...
  const load = async (append = false) => {
    ...
    const params = new URLSearchParams({ ...filters, limit: '48', offset: String(nextOffset) });
    try {
      const response = await fetch(`${API}/videos?${params}`);
      const next = (await response.json()) as { items: Video[]; total: number };
      setData((current) => (append ? { ...next, items: [...current.items, ...next.items] } : next));
      setOffset(nextOffset + next.items.length);
      setSelected(
        (current) =>
          next.items.find((item) => item.id === current?.id) ??
          (append ? current : (next.items[0] ?? null)),
      );
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (!node || loading || data.items.length === 0 || offset >= data.total) return;
