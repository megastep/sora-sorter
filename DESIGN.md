...
Cards are poster-first. Clicking a poster selects it for editing; the play overlay opens playback. The selected state uses the primary outline and tint, not a new color. Media itself may use black as a neutral viewing surface. Card status is visual: show read-only Signal Blue stars when a rating exists and a Signal Blue bookmark when favorited; reserve the status-row height when neither applies so the grid remains aligned.
